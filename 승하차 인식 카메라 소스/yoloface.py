...
parser.add_argument('--model-weights', type=str,
                    ...
                    # './model-weights/yolov3-wider_16000.weights' './model-weights/get_models.sh'
...
def _main():
    ...
    while True:
        ...
        faces = post_process(frame, outs, CONF_THRESHOLD, NMS_THRESHOLD, centerPoints, passed, standLineX)
        ...
        cv2.line(frame, (standLineX,100), (standLineX,600), COLOR_GREEN, 2)
        for (i, (txt, val)) in enumerate(info):
            text = '{}: {}'.format(txt, val)
            cv2.putText(frame, text, (10, (i * 20) + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_RED, 2)
            cv2.putText(frame, "Passed passenger : " + str(passed[-1]), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5 ,COLOR_GREEN, 2)
